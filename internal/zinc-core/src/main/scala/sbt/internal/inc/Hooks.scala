...
object Hooks {
  ...
  def addQuickAPI(m: java.util.Map[String, Object], f: (File, String) => Optional[AnalyzedClass]): Unit = {
    m.put(QUICK_API, f)
  ...
  def addInvalidationProfiler(m: java.util.Map[String, Object], invalidationProfiler: InvalidationProfiler): Unit = {
    ...
    Option(o) match {
      case Some(fj0: BiConsumer[_, _]) =>
        ...
        (cycle: Int, analysis: Option[CompileAnalysis]) =>
          fj.accept(cycle, analysis.fold[Optional[CompileAnalysis]](Optional.empty())(Optional.of(_)))
      case None =>
        (_, _) => None
    }
  }
  ...
  def addAnalysisPeek(m: java.util.Map[String, Object], f: (Int, Option[Analysis]) => Unit ): Unit = {
